...
public class MiddleWareImpl implements MiddleWare{
     ...
     * @param message 发布的信息
     */
    public BaseResponse<String> addMegToMQ(String queueId, String message) {
        //检测队列的ID合法性
     ...
     * @return 返回响应
     */
    public  BaseResponse<String> getMsgFromMQ(String queueId) {
        if (!msgQueues.containsKey(queueId)) {
     ...
     * @return 返回响应
     */
    public  BaseResponse<String> appendToTopic(String topic, String message, Long surTime) {
        //检测topic的合法性
        if (topic == null || topic.isEmpty()) return ResultUtils.error(ErrorCode.PARAMS_ERROR, "topic不能为空!");
        //如果主题不存在，那么就直接向主题中添加一个消息
        ...
        //如果待发送集合里没有这个topic，那么就新建一个，并添加所有订阅者
        else {
            //获取订阅此topic的所有user
            Vector<String> users = topicUser.get(topic);
            //把每个user都添加到相应的待发送集合中去
     ...
     * @return 返回响应
     */
    public  BaseResponse<String> getMsgFromTopic(String consumerId, String topicId) {
        //如果注册的topic不存在
     ...
     * @return 返回消息
     */
    public BaseResponse<String> addMsg(String message) {
     ...
     * @return
     */
    public  BaseResponse<String> addModule(String moduleId) {
